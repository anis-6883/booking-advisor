...
<nav class="page-breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"> <a class="text-muted" href="{{ route('super_admin.roles.index') }}">Role</a></li>
            ...
            <div class="col-md-12 stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <hr>
                        <form action="{{ route('super_admin.roles.update', $role->id) }}" method="POST" autocomplete="off">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="form-check mb-3">
                                        <input type="checkbox" class="form-check-input" id="checkPermissionAll" {{ App\Models\RolePermission::roleHasPermission($role->id, $all_permissions) ? "checked" : null }}>
                                    ...
                                    @foreach ($permission_groups as $group)
                                        <div class="row">
                                            @php
                                                $permissions = App\Models\Permission::where('group_name', $group->group_name)->get();
                                                $roleHasPermissions = App\Models\RolePermission::roleHasPermission($role->id, $permissions);
                                                $j = 1;
                                            @endphp
                                            <div class="col-md-3">
                                                <div class="form-check mb-3">
                                                    <input type="checkbox" class="form-check-input" id="{{ $i }}Management" {{ $roleHasPermissions ? "checked" : null }} onclick="checkPermissionByGroup('role-{{ $i }}-management-checkbox', this)">
                                            ...
                                            <div class="col-md-9 role-{{ $i }}-management-checkbox">
                                                ...
                                                <div class="form-check mb-3">
                                                    <input name="permissions[]" type="checkbox" class="form-check-input" {{ \App\Models\RolePermission::hasPermissionTo($role->id, $permission->id) ? 'checked' : ''  }} id="checkPermission{{ $permission->id }}" value="{{ $permission->id }}" onclick="checkSinglePermission('role-{{ $i }}-management-checkbox', '{{ $i }}Management', {{ count($permissions) }})">
